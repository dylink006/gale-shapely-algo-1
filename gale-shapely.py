...
def parse_input():
    if len(sys.argv) > 1:
        input_lines = open(sys.argv[1]).read().strip().splitlines()
    else:
        input_lines = []
        try:
            n = int(input().strip())
...
def gale_shapely(hospitals, applicants):
    ...
    while unmatched_hospitals.qsize() > 0:
        ...
        for i, a in enumerate(curr_hospital.prefs[start_index:]):
            ...
            if applicant_ranks[a][curr_hospital.id] < applicants[a][matched_applicants[a].id]:
...
if __name__ == "__main__":
    ...
    gale_shapely(hospitals, applicants)